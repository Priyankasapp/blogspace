...
export default async function ShopProductPage({ params }: Props) {
  ...
  // Strip non-numeric text out of the price field for safe numeric schema formatting (e.g., "$49" -> 49)
  const numericPrice = parseFloat(productData.price.replace(/[^0-9.]/g, ""));

  // 1. Generate the isolated single Product Structured Schema Object
  const productJsonLd = {
    "@context": "https://schema.org",
    "@type": "Product",
    "name": productData.title,
    // Safely parse static image data strings or objects if available
    "image": typeof productData.imageSrc === "string" ? productData.imageSrc : "",
    "description": productData.description,
    "category": productData.category,
    "offers": {
      "@type": "Offer",
      "url": `https://blogspace-jet.vercel.app/shop/${category}/${productSlug}`,
      "priceCurrency": "USD",
  ...
  return (
    <>
      {/* 2. Injecting the structural product data node for indexing spiders */}
      <script
