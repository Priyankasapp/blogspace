...
// Import your centralized data structure configurations
    ...
    <section ref={container} className="w-full bg-white px-[4%] py-16 flex justify-center">
      <div className="max-w-7xl w-full">
        ...
        <div className="flex justify-between items-baseline border-b border-neutral-200 pb-4 mb-12">
          ...
          <a href="#" className="text-[0.75rem] font-medium tracking-[1.5px] uppercase text-neutral-500 hover:text-black transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-16">
          {CURATED_PRODUCTS.map((product) => (
            <div key={product.id} className="animate-product-card opacity-0 flex flex-col w-full group">
              <div className="bg-[#f0f0f0] p-12 aspect-[4/5] flex items-center justify-center relative overflow-hidden mb-6">
                <div className="relative w-full h-full shadow-md transition-transform duration-500 group-hover:scale-105">
                  <Image
                    src={product.imageSrc} // Pulls seamlessly from assets config pointer
                    alt={product.title}
                    fill
              ...
              <div className="flex justify-between items-center mb-3">
                ...
                <button aria-label="Save item" className="text-neutral-400 hover:text-black transition-colors">
                  <Bookmark className="w-3.5 h-3.5 stroke-[1.5]" />
                </button>
              </div>

              <h3 className="font-serif font-bold text-2xl text-black tracking-tight mb-2">
                {product.title}
              </h3>
              ...
              <a href={product.link} className="w-full text-center py-3 border border-neutral-800 text-sm font-medium text-black bg-transparent transition-all duration-300 hover:bg-black hover:text-white">
                View Product
              </a>
            </div>
          ))}
